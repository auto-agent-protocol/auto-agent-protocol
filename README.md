...
[![Spec version](https://img.shields.io/badge/spec-v1.0.0-2563eb)](https://autoagentprotocol.org/docs/latest/intro)
[![A2A](https://img.shields.io/badge/A2A-v1.0-1d4ed8)](https://a2a-protocol.org)
...
![agent-card.json — the contract every AAP dealership exposes, advertising A2A v1.0 compliance, the AAP automotive extension, the subset of AAP skills the agent implements, and supported bindings](static/img/v1.0/agent-card-passport.png)

## v1.0.0 Scope

v1.0.0 is the **first stable release**. Its payload shape is the one introduced in v0.2, unchanged — a single `agent-card.json` is the only file a dealer publishes, prices are plain integers, the vehicle and dealer shapes are flat, and `status` is a controlled enum — now riding the released A2A v1.0 wire (`SendMessage`, `ROLE_USER`/`ROLE_AGENT`, `supportedInterfaces[]`). v0.1 and v0.2 remain published and frozen for anyone pinned to them.
...
v1.0.0 does **not** cover: authentication (agents are public by default; auth is left to A2A's native `securitySchemes`), payments, financing approval, RFQ/quote flows, trade-in valuations, or reservations.

![How an AI agent buys a car — discover via /.well-known/agent-card.json, browse with inventory.search, inspect with inventory.vehicle, and submit a unified lead.submit carrying customer + vehicle of interest + trade-in + appointment](static/img/v1.0/buyer-journey.png)

## Quick links

- **Specification**: [autoagentprotocol.org](https://autoagentprotocol.org)
- **Example agent card** (the single file a dealer deploys): [`spec/v1.0/examples/agent-card.example.json`](spec/v1.0/examples/agent-card.example.json)
- **JSON Schemas**: [`spec/v1.0/schemas/`](spec/v1.0/schemas/)
- **Examples**: [`spec/v1.0/examples/`](spec/v1.0/examples/)
- **OpenAPI 3.1** (built at deploy time): `https://autoagentprotocol.org/v1.0/openapi-jsonrpc.yaml`, `https://autoagentprotocol.org/v1.0/openapi-rest.yaml`
...
![AAP SDK — three open-source TypeScript packages: @autoagentprotocol/types, @autoagentprotocol/schemas, and @autoagentprotocol/validator](static/img/v1.0/aap-sdk-toolbox.png)
...
```
spec/v1.0/schemas/         JSON Schema 2020-12 source of truth — current version (committed)
spec/v1.0/examples/        Example payloads (committed)
spec/v1.0/skills.yaml      Skills manifest (committed)
spec/v0.1/, spec/v0.2/     Frozen released specs, kept for consumers pinned to them (committed, immutable)
docs/                      Hand-written documentation pages for the current version, v1.0.0 (committed)
versioned_docs/, versioned_sidebars/, versions.json  Frozen v0.1 + v0.2 docs snapshots (committed)
...
- Specification and schemas: [Apache-2.0](LICENSE) — chosen (and kept at 1.0.0) for its explicit patent grant, which protects every adopting dealership, platform, and SDK vendor; it is also the license of A2A itself.
