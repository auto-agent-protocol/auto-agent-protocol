...
3. `supportedInterfaces[]` includes an entry whose `protocolBinding` is `JSONRPC` (REQUIRED on every AAP agent card). An additional entry with `protocolBinding` `HTTP+JSON` MAY be added (OPTIONAL). gRPC is out of scope for AAP v1.0.
...
AAP v1.0 agents are **public by default** — the simplest setup needs no authentication. AAP defines no auth of its own. A dealer that wants to protect its endpoint uses A2A's native `securitySchemes` / `securityRequirements` on the agent card (e.g. HTTP bearer), and buyer agents obtain credentials out of band, exactly as A2A specifies. Auth is therefore an A2A/transport concern, out of scope for the v1.0 profile beyond what A2A already provides.

## Full example agent card

This is the **smallest** card that satisfies the three requirements above — a public dealer agent on one binding. Copy it, change the `name`, the `supportedInterfaces[].url`, and `params.id`, and keep only the `skills[]` you actually implement. A copy-pasteable copy is published at [`/v1.0/examples/agent-card.example.json`](https://autoagentprotocol.org/v1.0/examples/agent-card.example.json).
    ...
    "extensions": [
      {
        ...
        "description": "Auto Agent Protocol v1.0 — A2A Automotive Retail Profile.",
        "required": true,
        "params": {
...
Each skill carries the A2A-required `tags` (keywords clients/LLMs use to categorize and rank skills). Everything else is **optional** A2A surface a dealer MAY add to the same card — `documentationUrl`, an extra `supportedInterfaces` entry (e.g. `HTTP+JSON`), per-skill `examples` / `inputModes`, or `securitySchemes` + `securityRequirements` for auth. Note that the optional A2A surface beyond `SendMessage` (streaming, tasks, push notification configs, extended agent card) is out of scope for AAP v1.0 — dealer agents do not need to implement it and buyer agents MUST NOT require it. The AgentCard shape is A2A's; see the [A2A spec](https://a2a-protocol.org/latest/specification/).
...
2. Pick a binding from `supportedInterfaces[]`. A `JSONRPC` entry is always present on an AAP card; `HTTP+JSON` may also be offered. Both accept the same AAP `DataParts`; choose whichever the buyer agent already speaks.
3. Invoke skills via standard A2A `SendMessage` ([JSON-RPC binding](./bindings/json-rpc.md) or [REST binding](./bindings/rest.md)) — the only A2A operation AAP uses: request `Message` in, response `Message` out. If the card declares A2A `securitySchemes`, obtain credentials out of band first.
