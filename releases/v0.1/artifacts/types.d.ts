...
/**
 * FTC-emphasized FINAL out-the-door price after all incentives, mandatory fees, and required add-ons. Per FTC enforcement (CARS Rule), this field MUST reflect the total amount the buyer would pay; advertising a 'price' that excludes required fees or omits required add-ons is a violation. Inventory context.
 */
...
/**
 * Dealer's advertised list price; the base price BEFORE incentives, taxes, or fees. Inventory context.
 */
...
/**
 * Regional price equal to list_price plus applicable taxes for the buyer's `zip`, when the dealer enables desking. Present only if a `zip` is supplied AND the dealer supports regional pricing. Inventory context.
 */
...
/**
 * An appointment request the buyer wants scheduled alongside this lead. Optional. The vehicle for the appointment is implicit: `vehicle_of_interest` for test drives / handover, `trade_in` for trade-in appraisals.
 */
...
export interface Filters {
  ...
  year_max?: number;
  /**
   * Minimum price in the dealer's primary currency, applied against the FTC-final 'price' field.
   */
  price_min?: number;
  /**
   * Maximum price applied against the FTC-final 'price' field.
   */
  price_max?: number;
  mileage_max?: number;
  vin?: string;
...
export interface SkillBinding {
  id: "dealer.information" | "inventory.facets" | "inventory.search" | "inventory.vehicle" | "lead.submit";
  request_schema: string;
  response_schema: string;
  /**
   * Whether the skill MAY be invoked without authentication or consent.
   */
  anonymous_allowed: boolean;
  /**
   * Whether the skill REQUIRES a ConsentGrant when customer contact info is included.
   */
  consent_required: boolean;
  /**
   * Whether this skill emits an ADF/XML-mappable lead (only meaningful for lead.* skills).
   */
...
export interface Skill {
  /**
   * Skill identifier. AAP standard skill IDs (the v0.1 vocabulary): `dealer.information`, `inventory.facets`, `inventory.search`, `inventory.vehicle`, `lead.submit`. Agents MAY declare any subset of these (one or more); they MAY also declare additional non-AAP skill IDs alongside, which buyer agents will treat as agent-specific extensions.
   */
  id: string;
  name: string;
  description: string;
