...
The path `https://autoagentprotocol.org/latest/` always aliases the highest released version. It is intended for browsing convenience, not for production wire pinning. Production agents SHOULD pin to a specific version (e.g. `v1.1`).

How it is wired up today:

- **Documentation**: `https://autoagentprotocol.org/docs/latest/<page>` redirects (HTTP 200 client-side) to `/docs/v1.1/<page>` via the Docusaurus client-redirects plugin. Every page reachable under `/docs/v1.1/` is also reachable under `/docs/latest/`. The frozen v0.1, v0.2, and v1.0 docs remain served at `/docs/v0.1/<page>`, `/docs/v0.2/<page>`, and `/docs/v1.0/<page>`.
- **Schemas, examples, OpenAPI, MCP manifest**: also served at `https://autoagentprotocol.org/latest/...` because the build pipeline mirrors `static/v1.1/` into `static/latest/`. The frozen v0.1, v0.2, and v1.0 artifacts remain served at `https://autoagentprotocol.org/v0.1/...`, `https://autoagentprotocol.org/v0.2/...`, and `https://autoagentprotocol.org/v1.0/...`.

| URL form | Stability | Use case |
|---|---|---|
| `https://autoagentprotocol.org/v1.1/...` | Frozen | Production pinning. |
...
| `https://autoagentprotocol.org/latest/...` | Tracks the highest released version (now v1.1) | Documentation browsing, not wire pinning. |
| `https://autoagentprotocol.org/next/...` | Reserved | The current `docs/` is now v1.1; `next` will hold the preview of the next version once work on it begins. |
...
- `https://autoagentprotocol.org/v1.1/schemas/inventory-search-request.schema.json`
- `https://autoagentprotocol.org/v1.1/schemas/vehicle.schema.json`
...
1. **Current (working)** — in-flight work lives in the current version directory (`spec/v1.1/`) and the Docusaurus `current` docs (`docs/`, served at `/docs/v1.1/`). Schemas and prose may change while the version is unreleased.
...
- Pin to a specific version (`https://autoagentprotocol.org/v1.1/`) in production.
...
- Develop changes against the current version directory (`spec/v1.1/`); cut them into a new version directory when you release.
