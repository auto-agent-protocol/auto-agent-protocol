...
2. **Wait for triage.** Maintainers label the issue (`spec`, `schema`, `docs`, `tooling`) and confirm the direction. For schema changes, the issue gets a target version (e.g. `v1.2-candidate`).
...
| Path | Purpose |
|---|---|
| `spec/v1.2/schemas/` | Current (v1.2) JSON Schemas — the source of truth. Edit here. |
| `spec/v1.2/examples/` | Current (v1.2) example payloads. Edit here. |
| `spec/v1.2/skills.yaml` | Current (v1.2) skill manifest. Edit here. |
...
| `docs/` | Docusaurus prose pages for the current version (v1.2). |
...
The following are intentionally out of scope for v1.2; pull requests adding them to v1.2 will not be accepted:
...
- gRPC binding. (AAP v1.2 uses a single transport — JSON-RPC 2.0 — and requires a JSONRPC interface on every agent card; the HTTP+JSON (REST) binding was [removed in v1.1](./bindings/rest.md). A2A's gRPC binding is out of scope.)
