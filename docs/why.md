...
AAP is the first published A2A profile written specifically for the automotive retail vertical, riding on A2A v1.0; v1.2 is the current release. Its goal is narrow: a buyer agent should be able to talk to any compliant dealer agent — Toyota, Honda, an independent used-car lot, a motorcycle or powersports store, a CDK/Reynolds-backed group — through identical typed messages, with consent, pricing, and ADF compatibility built in from day one. The unified `Vehicle` shape carries an optional `inventory_class` (`automobile` or `motorcycle`) plus class-specific fields, so the same five skills serve both car and motorcycle dealers. A generic electric-powertrain group (range, battery kWh, motor hp, 0-60, charge time, DC fast charge) covers electric cars and electric motorcycles alike.
