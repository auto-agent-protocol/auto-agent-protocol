...
export interface Facets {
  ...
  displacement_cc_range?: RangeFacet1;
  electric_range_mi_range?: RangeFacet2;
}
/**
...
/**
 * A2A v1.0 AgentCard carrying the AAP automotive-retail extension. Published at /.well-known/agent-card.json on a dealer-controlled domain. A2A v1.0 declares every transport in 'supportedInterfaces[]' (it replaced the earlier top-level 'url'/'preferredTransport'/'additionalInterfaces' and top-level 'protocolVersion'). AAP v1.2 uses a single transport: JSON-RPC 2.0. A compliant AAP dealer agent advertises a JSONRPC interface in 'supportedInterfaces[]' and does not advertise any other transport for AAP. To be a compliant AAP dealer agent, 'capabilities.extensions' MUST include an entry whose 'uri' equals 'https://autoagentprotocol.org/extensions/aap/v1.2'.
 */
export interface AgentCard {
  ...
  description: string;
  /**
   * The A2A v1.0 service interfaces (transport + endpoint). AAP v1.2 uses a single transport: a JSONRPC interface is REQUIRED and is the only transport an AAP agent advertises (every AAP client can rely on it). The HTTP+JSON (REST) binding was removed in v1.1; gRPC is out of scope. The protocolBinding enum still lists 'HTTP+JSON' for raw A2A compatibility, but AAP v1.2 advertises only 'JSONRPC'.
   *
   * @minItems 1
   */
  supportedInterfaces: [
  ...
  capabilities: {
    ...
    /**
     * Declared A2A extensions. MUST include the AAP automotive-retail v1.2 entry.
     */
