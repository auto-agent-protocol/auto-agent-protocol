...
export interface Vehicle {
  ...
  body?: string;
  /**
   * Transmission type as text. Automobiles use e.g. 'automatic', 'manual', '8-speed automatic', 'cvt'; motorcycles use e.g. 'manual', 'automatic', 'dct', 'semi-automatic'.
   */
  transmission?: string;
  /**
  ...
  /**
   * Manufacturer 0–60 mph acceleration time in seconds. Generic performance spec; most useful for electric units where it is a headline number (e.g. 3.0 for an electric standard motorcycle).
   */
  accel_0_60_mph_s?: number;
  /**
   * Approximate charge time in minutes for BEV/PHEV units. Charging conditions (Level 2 vs DC fast, and the start/target state of charge) SHOULD be stated in `description` or `features`, since a single number is otherwise ambiguous. E.g. 78 (electric standard motorcycle, Level 2).
   */
...
export interface Facets {
  ...
  displacement_range?: RangeFacet1;
}
...
export interface AgentCard {
  ...
  capabilities: {
    ...
    /**
     * Declared A2A extensions. MUST include the AAP automotive-retail v1.1 entry.
     */
